...
open class Coordinator<T: UIViewController>: UIResponder, Coordinating {
    ...
    /// UIResonder 中通过 next Responder 方法来传递事件, 但是要注意的是, 该方法默认返回都是nil,
    /// iOS UIView 系列控件中往上追溯父视图时都重写了该方法. 将返回的下一级响应者这是为了该控件的父视图
    /// 所以这里, 我们自己建立了一个UIResponder的变量 coordinatingResponder, 如果用next的话, 返回的就是nil, 所以应该返回父控件作为响应者
